...
* `PhotoEditViewController` fully supports the iPhone X.
...
* Fixed an issue where performance would decrease after tinting a sticker.
* Fixed an issue where a tinted sticker was not tinted in the exported image.
* Fixed an issue with the gestures in the focus tool while the editor was embedded in a `UINavigationController`.
* Fixed an issue where the color saturation and brightness picker would sometimes not update while dragging the hue picker.
* Fixed an issue where stickers and text had the wrong position in the exported image with iOS  9 and 10.
* Fixed an issue with a misaligned sticker and text selection view when the editor was embedded in a `UINavigationController`.
...
* The `forceCropMode` is not enabled in this build.
* Serialization and deserialization is not enabled in this build.
* Some analytics callbacks might not be triggered in this build.
* Accessibility is not working completely yet.
* Some `Configuration` closures currently have no effect.
* Certain API might not be exposed to Objective-C in this build.

### Added

* Full support for iOS 11 and Swift 4.
* Added rotation gesture support to `FrameToolController`.
* Added serialization format v3.0.0, which can be used to serialize and deserialize edits across all platforms (with certain limitations).

### Changed

* Completely refactored the sprite handling code. Sprites (stickers, text, brush and farmes) are now directly rendered on top of the image instead of rendering a view snapshot. This results in better looking images, better performance and more robust code.
